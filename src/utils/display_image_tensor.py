...
def display_mask_tensor(monolabel_mask_tensor: torch.Tensor) -> None:
    ...
    if len(labels) > 256:
        raise ValueError("the input mask contains too many labels to be displayed using this method!")
...
def display_multilabel_mask_tensor(multilabel_mask_tensor: torch.Tensor) -> None:
    """
    Display a multilabel mask (multi-channels) stored in a torch tensor as a mosaic, each tile of the mosaic corresponding
